...
function normSku(v: any) {
  // trim + collapse weird whitespace + uppercase for comparison only
...
function extractBuyBoxPrice(body: any): { amount: number | null; currency: string | null; numberOfOffers: number | null } {
  ...
  return { amount: Number.isFinite(a) ? round2(a) : null, currency: currency ? String(currency) : null, numberOfOffers };
}
...
const LIST_INVENTORYSKUS = /* GraphQL */ `
  query ListInventorySkus($filter: ModelInventorySkuFilterInput, $limit: Int) {
    listInventorySkus(filter: $filter, limit: $limit) {
      items { marketplaceId sku asin }
    }
  }
`;
...
export async function POST(req: Request) {
  try {
    ...
    const sku = safeStr(searchParams.get("sku"));
   ...
   // 1) Resolve ASIN (cheap, internal). Prefer non-SupplierMap sources when possible.
let asin = "";

// A) Try InventorySku (if schema supports asin)
try {
  const inv: any = await gql(LIST_INVENTORYSKUS, {
    filter: { marketplaceId: { eq: mid }, sku: { eq: sku } },
    limit: 1,
  });
  asin = safeStr(inv?.listInventorySkus?.items?.[0]?.asin);
} catch {
  // ignore if schema doesn't have asin
}

if (!asin) {
  // B) SupplierMap fallback (scan + normalization, as you already have)
  ...
  for (let page = 0; page < 10; page++) {
    ...
    for (const it of items) {
      if (normSku(it?.sku) === target) {
        asin = safeStr(it?.asin);
        break;
      }
    }
    if (asin) break;
...
if (!asin) {
  return NextResponse.json(
    { ok: false, error: "No ASIN for SKU (not found in InventorySku or SupplierMap)", sku },
    { status: 404 }
  );
}

    // 2) Call non-batch pricing offers (this is the STK-cheap, per-SKU truth fetch)
    const offers = await spapiFetch<any>({
      method: "GET",
    ...
    return NextResponse.json({ ok: true, mid, sku, asin, dryRun, truth: input });
  } catch (e: any) {
