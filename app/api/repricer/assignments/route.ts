// app/api/repricer/assignments/route.ts
...
export async function GET() {
  try {
    ...
    const raw = d?.getAppSettings?.repricerAssignmentsJson ?? "[]";
    return NextResponse.json({ ok: true, assignments: safeJson(raw, []) });
  } catch (e: any) {
...
export async function PUT(req: Request) {
  try {
    ...
    if (!assignments) return NextResponse.json({ ok: false, error: "Body must include assignments: []" }, { status: 400 });

    const nowIso = new Date().toISOString();
    const input = {
      id: "global",
      repricerAssignmentsJson: JSON.stringify(assignments),
      updatedAtIso: nowIso,
    };
