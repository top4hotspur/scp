...
async function gql<T>(query: string, variables?: any): Promise<T> {
  if (!DATA_URL || !DATA_API_KEY) {
    throw new Error(
      `Missing Amplify Data connection. DATA_URL=${DATA_URL ? "Y" : "N"} DATA_API_KEY=${
        DATA_API_KEY ? "Y" : "N"
      }`
    );
  }
...
export async function POST(req: Request) {
  try {
    ...
    const draftSuffix = String(body.draftSuffix ?? "").trim(); // required (stable bucket)
    ...
    if (!po?.id) throw new Error("Failed to create/load PurchaseOrder");
    ...
    const lines = (linesData?.listPurchaseOrderLineByPurchaseOrderIdAndSku?.items ?? []).filter(
      (x: any) => x && Number(x.qty) > 0
    );
    ...
    // 5) Update PO totals + timestamp
    // IMPORTANT: because PurchaseOrder has a composite @index byMarketplaceSupplierStatus,
    // updates must include ALL key fields for that index (marketplaceId, supplier, status).
