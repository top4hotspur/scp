...
function buildSupplierSkuAliasMaps(supplierSkus: string[]) {
  ...
  for (const raw of supplierSkus) {
    ...
    // if it ends with a 3-6 digit number, alias that too (extra safe)
...
export async function GET(req: Request) {
  try {
    ...
    // OPTIONAL supplier filter (empty => ALL suppliers)
    const supplier = String(searchParams.get("supplier") ?? "").trim();
    const supplierFilter = supplier ? supplier : null;

    // Requested horizon for cover/order math (UI), baseline is always 30d.
    const requestedDays = Math.max(2, Math.min(90, Number(searchParams.get("days") ?? 30)));
    const baseDays = 30;

    // Optional: extra windows for repricer / analysis (e.g. extra=2,7)
    const extraDays = String(searchParams.get("extra") ?? "")
      .split(",")
    ...
    // NOTE: pass the supplier SKUs we have; your availability route already handles alias resolution
    const stock = await loadAvailability(origin, mid, supplierSkus);

    // 3) Velocity from SalesLine across multiple windows (single scan)
    const maxWindow = windows[windows.length - 1] ?? 30;
    ...
    // Freshness:
    // - maxSalesAtIsoAll: latest timestamp for this marketplace (even if sku doesn't match supplier)
    // - maxSalesAtIsoMatched: latest timestamp among lines that matched supplier sku/alias
    ...
    do {
      ...
      for (const it of items) {
        ...
        // Ignore anything older than the largest window we’re computing
        if (at < fromIsoMin) continue;

        const qty = safeNum(it.qty);

        // Add qty into every window bucket that includes this timestamp
        for (const d of windows) {
          const startIso = fromIsoByDays.get(d)!;
          if (at >= startIso) {
    ...
    const rows = supplierSkus.map((fullSku) => {
      ...
      // Stock maps are keyed by the requested sku string from availability route
      ...
      // Baseline: always 30d
      const sold30 = safeNum(soldUnitsByDays.get(30)?.get(fullSku) ?? 0);
      const vel30 = sold30 / 30;

      // Requested window: cover/order calculations & table display
      const soldReq = safeNum(soldUnitsByDays.get(requestedDays)?.get(fullSku) ?? 0);
      const velReq = soldReq / requestedDays;

      // Optional extra windows (e.g. 2d/7d for repricer)
      const extras: Record<string, any> = {};
      for (const d of windows) {
        if (d === 30 || d === requestedDays) continue;
        const s = safeNum(soldUnitsByDays.get(d)?.get(fullSku) ?? 0);
        extras[`soldUnits${d}d`] = s;
        extras[`dailyVel${d}d`] = s / d;
      }

      const projectedSales = soldReq;
      const projectedBalance = stockForCover - projectedSales;

      const daysOfCover = velReq > 0 ? stockForCover / velReq : null;
      ...
      return {
        ...
        // Always-on baseline (repricer/analytics)
        soldUnits30: sold30,
        dailyVel30: vel30,
    ...
    // Prefer matched freshness, fall back to marketplace freshness
