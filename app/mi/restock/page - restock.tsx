...
type RestockRow = {
  ...
  unitCost: number;
  soldUnits: number; // <-- weâ€™ll display this as â€œSalesâ€
  dailyVel: number;
};
...
export default function Page() {
  ...
  const [sortDir, setSortDir] = useState<SortDir>("desc"); // default direction for first click
  ...
  function toggleSort(key: SortKey) {
    // Requirement: first click DESC, second click ASC, one active at a time
    setSortKey((prev) => {
      if (prev === key) {
  ...
  function sortIcon(key: SortKey) {
    if (sortKey !== key) return <ChevronDown className="h-4 w-4 opacity-40" />;
    return sortDir === "desc" ? (
...
case "daysToOrder": {
  ...
  }, [rows, sortKey, sortDir, stockAvailBySku, stockInboundBySku]);
